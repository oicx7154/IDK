...
local FetchIcons, Icons = pcall(function()
    return (loadstring(
        game:HttpGet("https://raw.githubusercontent.com/oicx7154/IDK/refs/heads/main/Assets.lua")
    ) :: () -> IconModule)()
end)

function Library:GetIcon(IconName: string): Icon?
    -- 确保数据已加载
    if not FetchIcons or type(IconsTable) ~= "table" then
        warn("灵构: 资源表尚未加载或加载失败")
        return nil
    end
    ...
    if not AssetId then
        -- 如果没找到，打印出来方便你排查
        warn("灵构: 未在资源表中找到图标 -> " .. tostring(IconName))
        return nil
    end
